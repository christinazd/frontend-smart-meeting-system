...
const getRoomImage = () => {
  const images = ['images/brr.jpg','images/mm.jpg','images/mr.jpg','images/hr.jpg'];
...
function parseCapacity(value) {
  switch(value) {
    case "2-4 people": return { min: 2, max: 4 };
    case "5-8 people": return { min: 5, max: 8 };
    case "9-12 people": return { min: 9, max: 12 };
    case "13+ people": return { min: 13, max: 9999 };
    default: return { min: 0, max: 9999 }; // Any
  }
}
...
function markBookedSlots(bookings) {
  ...
  bookings.forEach(m => {
    ...
    bookedSet.add(`${String(start.getHours()).padStart(2,'0')}:${String(start.getMinutes()).padStart(2,'0')}`);
  });
...
function renderRooms(rooms, selectedStartTime = null) {
  ...
  rooms.data.forEach(room => {
    ...
    if (selectedStartTime && !room.available_slots.includes(selectedStartTime)) return;
    ...
    const roomHtml = `
      <div class="col-md-6">
        <div class="card room-card">
          ...
          <img src="${room.image_url || getRoomImage()}" class="card-img-top room-img" alt="${room.name}">
          <div class="card-body">
            <h5 class="card-title">${room.name}</h5>
            <p class="card-text">${room.location || 'No location available.'}</p>
            <ul class="room-features">
              <li><i class="tf-ion-ios-people"></i> Room Capacity: ${room.capacity} people</li>
              ${room.features ? room.features.map(f => `<li><i class="${f.icon || 'tf-ion-ios-checkmark'}"></i> ${f.name}</li>`).join('') : ''}
            </ul>
            <button class="btn btn-main btn-sm btn-block"
              ...
              ${buttonDisabled}>${buttonDisabled ? 'Currently Booked' : 'Book Now'}</button>
          </div>
        </div>
      </div>
    `;
...
async function fetchRooms(filters = {}) {
  ...
  if (filters.capacity_min != null) params.append('capacity_min', filters.capacity_min);
  if (filters.capacity_max != null) params.append('capacity_max', filters.capacity_max);
  if (filters.feature_id) params.append('feature_id', filters.feature_id);

  const url = `http://127.0.0.1:8000/api/rooms?${params.toString()}`;
  try {
    const res = await fetch(url, {
      headers: {
        'Accept': 'application/json',
...
await populateEquipmentDropdown();
  ...
  async function populateSearchTimes(date) {
    ...
    populateTimeSelect(document.getElementById('startTime'), Array.from(allTimes).sort());
    populateTimeSelect(document.getElementById('endTime'), Array.from(allTimes).sort());
  }
  ...
  $('#confirmBooking').on('click', async function() {
    ...
    if (!roomId || !date || !start || !end || !title) {
      $('#bookingError').show().text('Please fill required fields (room/date/time/title).');
      return;
    }
    ...
    for (const m of existing) {
      ...
      if (!(newEnd <= s || newStart >= e)) {
        $('#bookingError').show().text(`Time overlaps with "${m.title || 'existing meeting'}" (${m.booking_start} - ${m.booking_end})`);
        return;
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        $('#bookingError').show().text(body.errors ? Object.values(body.errors).flat().join(' | ') : body.message || 'Booking failed');
      }
    } catch (err) {
      console.error(err);
